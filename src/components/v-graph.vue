  ...
  <section class="relative">
    ...
    >
      <div
        ...
        :style="{ height: `${bar}%`, width: `${$options.GRAPH_EL_WIDTH}px` }"
      ></div>
    </div>
...
export default {
  ...
  computed: {
    graph() {
      if (this.graphArr.length > this.maxGraphEl) {
        return [...this.graphArr].slice(this.graphArr.length - this.maxGraphEl);
      }
  ...
  methods: {
    ...
    calculateMaxGraphEl() {
      this.maxGraphEl = Math.floor(
        this.$refs.graph.clientWidth / this.$options.GRAPH_EL_WIDTH
      );
    }
  },
  mounted() {
